    ...
    return (
        <div className="space-y-6">
            ...
            <div className="relative max-w-md">
                ...
                <input type="text" placeholder="Search templates..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                    ...
                    {!search && <button onClick={() => navigate('/email-marketing/templates/new')} className="btn btn-primary">Create Template</button>}
                </div>
            ) : (
                        ...
                        <div key={t.id} onClick={() => navigate(`/email-marketing/templates/${t.id}/edit`)} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow cursor-pointer group">
                            <div className="h-36 bg-slate-50 dark:bg-slate-700 flex items-center justify-center relative">
                                ...
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={(e) => handleClone(t.id, e)} className="w-7 h-7 rounded bg-white dark:bg-slate-600 shadow text-slate-600 dark:text-slate-300 flex items-center justify-center hover:bg-indigo-50" title="Clone"><FiCopy className="w-3.5 h-3.5" /></button>
